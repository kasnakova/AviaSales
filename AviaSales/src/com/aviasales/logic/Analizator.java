...
public class Analizator {
		...
		for (Flight f : flights) {
			if ((f.getDep().toLowerCase().equals(dep.toLowerCase())) && (f.getDepTime().toLocalDate().isEqual(depDate)
				...
				if (isEndFlight(f, arr)) {
					result.add(new Route(f));

				} else {
					SearchConnectedFlights(f, arr, sp.getPersonCount(), result);
				}
			}
		}
	...
	private static void SearchConnectedFlights(Flight flight, String arr, int personCount, List<Route> result)
			throws IOException {
		ArrayList<Flight> flights = StorageAdapter.loadFlights();
	...
	public static boolean isConnectingFlight(Flight f1, Flight f2) {
		...
		if ((f1.getArr().toLowerCase().equals(f2.getDep().toLowerCase())) && (f2.getDepTime().isAfter(minDate))
				&& (f2.getDepTime().isBefore(maxDate))) {
			return true;
		} else {
			return false;
		}
